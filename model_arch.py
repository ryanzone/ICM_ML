...
# TRAINING FUNCTION
def train_model(model, dataloaders, sizes, criterion, optimizer, epochs=5):
    train_acc_history = []
    val_acc_history = []
    best_acc = 0.0
...
# EVALUATION FUNCTION
def evaluate_model(model, val_dl, threshold=0.35):
    print(f"Evaluating model performance with PNEUMONIA THRESHOLD = {threshold}")
    model.eval()
    ...
    y_pred_tuned = (y_probs[:, 1] >= threshold).astype(int)
...
if __name__ == "__main__":
    # Prepare data
    train_paths, valid_paths, train_labels, valid_labels = prepare_data()
    ...
    # Train model
    model, train_acc, val_acc = train_model(model, dataloaders, sizes, criterion, optimizer, epochs=EPOCHS)
    
    # Evaluate model
    evaluate_model(model, val_dl, threshold=0.35)